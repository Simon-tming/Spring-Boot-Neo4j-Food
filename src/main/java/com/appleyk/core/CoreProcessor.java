...
public class CoreProcessor {
    ...
    public List<String> analysis(String querySentence) throws Exception {
      ...
      // System.out.println("结果00："+ Arrays.toString(str1));



       List<String> resultList = new ArrayList<>();
        resultList.add(String.valueOf(modelIndex));
        String finalPattern1=finalPattern.trim();
        String[]  finalPatternArr = finalPattern1.split(" ");
       // for (int h=0;h<finalPatternArr.length;h++) {
           // System.out.println("结果1：" + finalPatternArr[0]);
        resultList.add(finalPatternArr[0]);
    ...
    public  String queryAbstract(String querySentence) {
        ...
        for (Term term : terms) {
            ...
            if (termStr.contains("nf")) {        //nf 食物名
                abstractQuery += "nf ";
                abstractMap.put("nf", word);
            }
            else if (termStr.contains("ns")) {  //ns  症状
                abstractQuery += "ns ";
                abstractMap.put("ns", word);
    ...
    public Map<String,Integer> loadVocabulary() {
        ...
        File file = new File(rootDirPath + "custom/vocabulary.txt");
    ...
    public NaiveBayesModel loadClassifierModel() throws Exception {
        ...
        String[] sentences;
        Map<Double, String> seqWithSamples = loadQuestionSamples("custom/question");
    ...
    public Map<Double,String> loadQuestionTemplates() {
        Map<Double, String> questionsPattern = new HashMap<>(10);
        File file = new File(rootDirPath + "custom/question_classification.txt");
        BufferedReader br = null;
        try {
            br = new BufferedReader(new FileReader(file));
